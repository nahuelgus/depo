...
$depos = DB::all("SELECT id,nombre FROM depo_deposits WHERE is_activo=1 ORDER BY id");

$cond = "WHERE t.fecha BETWEEN :d1 AND :d2";
$params = [':d1'=>"$desde 00:00:00", ':d2'=>"$hasta 23:59:59"];
if($dep>0){
  $cond .= " AND (t.from_deposit_id=:dep OR t.to_deposit_id=:dep)";
  $params[':dep'] = $dep;
}
...
<nav class="navbar navbar-light border-bottom px-3">
  ...
  <div class="d-flex gap-2">
    <a href="/app/deposito/public/transfers_new.php" class="btn btn-sm btn-primary">Nueva</a>
    ...
    <div class="col-md-4">
      ...
      <select name="dep" class="form-select">
        ...
        <?php foreach($depos as $d): ?>
          <option value="<?=$d['id']?>" <?=$dep===$d['id']?'selected':''?>><?=h($d['nombre'])?></option>
        <?php endforeach; ?>
      </select>
    </div>
      ...
      <tbody>
        <?php foreach($rows as $r): ?>
          <tr>
            ...
            <td><a class="btn btn-sm btn-outline-secondary" href="/app/deposito/public/transfers_view.php?id=<?=$r['id']?>">Ver</a></td>
          </tr>
        <?php endforeach; ?>
        <?php if(!count($rows)): ?>
